...
public class ParkingMapTest_2place {
    ...
    public void testAddParkingSpot() {
        ...
        assertEquals("Должно быть добавлено одно парковочное место",
                2, parkingMap.getParkingSpots().size());
    ...
    public void testAddsParkingSpot() {
        ParkingSpot spot = new ParkingSpot("B2", false);
        parkingMap.addParkingSpot(spot);
        parkingMap.addParkingSpot(spot);

        assertEquals("Количество мест должно быть 2", 2, parkingMap.getParkingSpots().size());
        assertEquals("Добавленное место должно быть 'B2'", spot, parkingMap.getParkingSpots().get(0));
    }
    ...
    public void testMultipleParkingSpots() {
        ...
        assertEquals("Должно быть 5 парковочных мест",
                6, parkingMap.getParkingSpots().size());
